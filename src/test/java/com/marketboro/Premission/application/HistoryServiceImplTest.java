...
public class HistoryServiceImplTest {
    ...
    public void 포인트적립및사용내역조회성공() {
        ...
        // when
        List<History> result = historyServiceImpl.getPagedUsageHistoryByMemberId(testMember.getMemberId(), testMember.getMemberName(), pageable);

        // then
        verify(memberRepository, times(1)).findByMemberId(testMember.getMemberId());
        verify(historyRepository, times(1)).findByMemberMemberIdOrderByHistoryDateDesc(testMember.getMemberId(), pageable);
